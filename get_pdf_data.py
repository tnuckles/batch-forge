...
def friendly_name(pdf):
    friendly_pdf_name = (
        order_number(pdf) + " " + order_item(pdf) + " " + template_name(pdf)
    )
...
def due_date(pdf):
    ...
    return datetime.date(
        datetime.strptime(file_name.split("(")[1].split(")")[0], "%Y-%m-%d")
    )
...
def odd_or_even(pdf: str) -> int:
    """
    Accepts a print PDFs filepath as a string. Strips down to quantity, then
    applies % 2 to determine if it's odd or even

    Returns an int as either 0 or 1.
    """
    # filename = name(pdf)
    # print(filename)
    # filename_2 = filename.split('-')[9]
    # print(filename_2)
    # filename_3 = filename_2.split('Qty ')[1]
    # print(filename_3)
    # odd_or_even = int(filename_3) % 2
    # print(odd_or_even)
    # return odd_or_even
...
def calculate_length(quantity, height):
    ...
    first = math.floor(
        quantity / 2
    )  # quantity divided by two, rounded down, to get the number of panels we can fit side by side
    second = first + (quantity % 2)  # quantity + 1 for any odd-quantitied items
    third = height + 0.5  # height + .5 because there's a .5" gap between each row
    length = second * third
    return length
