...
def splitMultiPagePDFs(print_pdf):
    ...
    if num_of_pages > 1:
            ...
            for n, page in enumerate(pdf.pages):
                ...
                dst.save(namePt1 + str(quantity) + '-' + template_name + ' Panel ' + str(n + 1) + namePt2)
            try:
                os.remove(print_pdf)
...
def crop_multipanel_pdfs(print_pdf_to_split, batch_dir):
    ...
    order_dict['copped_pdf_name'] = order_dict['file_name'].split(order_dict['template_name'])[0] + order_dict['template_name'] + ' Split' + order_dict['file_name'].split(order_dict['template_name'])[1] + '.pdf'
    
    if '(OTP' in get_pdf.name(order_dict['template_name']):
        OT_Panel = get_pdf.name(order_dict['template_name']).split('(OTP')[1].split(')')[0]
        quantity_per_panel_dict = determine_panel_quantity(order_dict['quantity'], order_dict['repeat'], OT_Panel)
    else:
        quantity_per_panel_dict = determine_panel_quantity(order_dict['quantity'], order_dict['repeat'])
    ...
    for page in range(order_dict['repeat_panels']):
        ...
        for crop_count in range(order_dict['repeat_panels']):
            ...
            lowerLeftX += 1728
            upperRightX += 1728
            printPDFName = batch_dir + '/' + order_dict['order_number'] + '-' + order_dict['order_item'] + '-' + str(crop_count + 1) + '.pdf'
            if printPDFName in order_dict['multi_page_pdfs']:
                continue
            else:
                order_dict['multiPagePDFs'].append(printPDFName)
    ...
    for PDF in order_dict['multi_page_pdfs']:
        ...
        panelNum = order_dict['template_name'] + ' TQ' + str(order_dict['quantity']) + ' P' + str(pageNum + 1)
        newName = newNamePt1 + panelNum + newNamePt2 + '.pdf'
        
        new_quantity_name_pt1 = newName.split('Qty ' + str(order_dict['quantity']))[0] + 'Qty '
        ...
        if final_name in order_dict['PDFPanelsToCombine']:
            continue
        else:
            order_dict['PDFPanelsToCombine'].append(final_name)

        with open(final_name, 'wb') as outputPDF:
            writer.write(outputPDF)
    
    # splitAndCombinedPDF = combineSplitPDFS(order_dict['PDFPanelsToCombine'], order_dict['copped_pdf_name'])

    for PDF in order_dict['multiPagePDFs']:
        os.remove(PDF)
