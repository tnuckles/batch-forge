...
def sort_pdfs_by_order_number(pdf_list: list):
    """
    Takes a list of pathstopdfs and sorts them by order number, from least to greatest.
    """
    list_to_sort = []
    sorted_list = []
    for print_pdf in pdf_list:
        order_number = get_pdf.order_number(print_pdf)
        list_to_sort.append((order_number, print_pdf))
    list_to_sort.sort(reverse=False, key=lambda pdf: pdf[0])
    pdf_list = list_to_sort
    list_to_sort = []
    for print_pdf in pdf_list:
        sorted_list.append(print_pdf[1])
    pdf_list = sorted_list
    sorted_list = []
    return pdf_list


def calculate_sample(list_of_pdfs: list) -> int:
    """
    Accepts a list of samples, then calculates their full length.
    Returns an int.
    """
    count = len(list_of_pdfs)
    return (floor(count / 2) + (count % 2)) * 9.5


def calculate_full_length(glob_of_pdfs: list) -> int:
    """
    Accepts list of PDFs, calls sortPdfsByLength to sort them by length, then
    ...
    for print_pdf in sorted_list:
        pdfLength = get_pdf.length(print_pdf)
        ...
        if find_odd is False:
            ...
            if pdf_odd_or_even == 1:
                if odd_match_height == pdf_height:
                    total_pdf_length += pdfLength - (pdf_height + 0.5)
                    find_odd = False
                else:
                    total_pdf_length += pdfLength
                    odd_match_height = pdf_height
...
def sort_pdf_list_by_odds(pdf_list: list) -> dict:
    ...
    for print_pdf in pdf_list:
        pdfOdd = get_pdf.odd_or_even(print_pdf)
        if pdfOdd == 1:
...
def sort_pdfs_by_item_number(pdf_list: list) -> list:
    """
    Takes a list of pathstopdfs and sorts them by order item number, from least
    to greatest. Currently not used in this file, but only called by
    caldera_importing_logic. Leaving here for organization.
    """

    list_to_sort = []
    sorted_list = []
    for print_pdf in pdf_list:
        pdf_item_num = int(get_pdf.order_item(print_pdf))
        list_to_sort.append((pdf_item_num, print_pdf))
    list_to_sort.sort(reverse=False, key=lambda pdf: pdf[0])
    pdf_list = list_to_sort
    list_to_sort = []
    for print_pdf in pdf_list:
